...
bool
MCS51FrameLowering::spillCalleeSavedRegisters(MachineBasicBlock &MBB,
  ...
  MCS51MachineFunctionInfo *MFI = MF.getInfo<MCS51MachineFunctionInfo>();
  ...
  for (unsigned i = CSI.size(); i != 0; --i) {
    ...
    MBB.addLiveIn(Reg);
    BuildMI(MBB, MI, DL, TII.get(MCS51::PUSH16r))
...
bool
MCS51FrameLowering::restoreCalleeSavedRegisters(MachineBasicBlock &MBB,
                                                 MachineBasicBlock::iterator MI,
                                        const std::vector<CalleeSavedInfo> &CSI,
                                        const TargetRegisterInfo *TRI) const {
  if (CSI.empty())
  ...
  for (unsigned i = 0, e = CSI.size(); i != e; ++i)
    BuildMI(MBB, MI, DL, TII.get(MCS51::POP16r), CSI[i].getReg());
...
void
MCS51FrameLowering::processFunctionBeforeFrameFinalized(MachineFunction &MF,
                                                         RegScavenger *) const {
